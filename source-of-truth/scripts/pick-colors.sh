...
# A bright, high-contrast "safety palette" to ensure readability in Kitty.
SAFETY_PALETTE=(
    "#f8f8f2" # FG
    "#bd93f9" # Accent 1
    "#ff79c6" # Accent 2
...
ACCENT1_RGB=$(hex_to_rgb "$ACCENT1")
ACCENT2_RGB=$(hex_to_rgb "$ACCENT2")
ROFI_BG_TRANSPARENT="argb:DB${BG#"#"}"
ROFI_INPUTBAR_TRANSPARENT="rgba(${ACCENT1_RGB}, 0.5)"
...
# 5. Generate the qt5ct/qt6ct color scheme
...
highlight_text=${BG}
link=${ACCENT2}
link_visited=${ACCENT2}

[Palette]
active_window=${ACCENT1}
active_highlight=${ACCENT2}
active_button=${ACCENT1}
active_base=${BG}
active_text=${FG}
active_link=${ACCENT2}
active_visited_link=${ACCENT2}
inactive_window=${BG}
inactive_highlight=${ACCENT1}
inactive_button=${BG}
inactive_base=${BG}
inactive_text=${FG}
inactive_link=${ACCENT2}
inactive_visited_link=${ACCENT2}
disabled_window=${BG}
disabled_highlight=${BG}
disabled_button=${BG}
disabled_base=${BG}
disabled_text=${ACCENT1}
disabled_link=${ACCENT1}
disabled_visited_link=${ACCENT1}
EOF
...
inputbar {
    ...
    margin-bottom: 10px;
    border-radius: 8px;
    background-color: ${ROFI_INPUTBAR_TRANSPARENT};
    text-color: ${FG};
}
...
# 8. Generate the Kitty theme.conf using the Hybrid Theming Engine
# The script will use bright colors from the SAFETY_PALETTE if the wallpaper colors are too dark.
kitty_palette=()
# Create a temporary array of wallpaper colors, excluding the background.
temp_palette=()
for color in "${palette[@]}"; do
    if [[ "$color" != "$BG" ]]; then
        temp_palette+=("$color")
...
# Build the final Kitty palette, replacing dark colors with safe ones.
for i in {0..7}; do
    wallpaper_color="${temp_palette[$i]:-${SAFETY_PALETTE[$i]}}"
    brightness=$(get_brightness "$wallpaper_color")
    if (( brightness < 128 )); then
        kitty_palette+=("${SAFETY_PALETTE[$i]}")
    else
        kitty_palette+=("$wallpaper_color")
    fi
done
...
foreground ${kitty_palette[0]}
background ${BG}
cursor ${kitty_palette[1]}
selection_foreground ${BG}
selection_background ${kitty_palette[1]}
...
# Red
color1 ${kitty_palette[3]}
color9 ${kitty_palette[3]}

# Green
color2 ${kitty_palette[4]}
color10 ${kitty_palette[4]}

# Yellow
color3 ${kitty_palette[5]}
color11 ${kitty_palette[5]}

# Blue
color4 ${kitty_palette[1]}
color12 ${kitty_palette[1]}

# Magenta
color5 ${kitty_palette[2]}
color13 ${kitty_palette[2]}

# Cyan
color6 ${kitty_palette[6]}
color14 ${kitty_palette[6]}

# White
color7 ${kitty_palette[0]}
color15 ${kitty_palette[0]}
EOF
